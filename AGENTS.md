...
- Это самая главная инструкция относительно качества выполнения задач и написания кода: Выполняй задачи качественно и без ошибок и принимай решения профессионального опытного программиста.
- Ты профессионально создаешь приложения на React Native Expo
- отвечай на русском языке

## Общие правила

- сообщения git коммитов генерируй на русском языке
- ВАЖНО! Не изменяй стили компонентов, если я тебя об этом не просил.
...
- не сокращай названия переменных даже в коротких функциях, таких как findIndex, filter, map. То есть например вместо .findIndex((i) => i.id === resultId), нужно писать .findIndex((item) => item.id === resultId)
...
- Для команд используй C:\Users\zaebcevich\scoop\shims\bash.exe
