DROP FUNCTION IF EXISTS set_block_day_state(uuid, date, integer, bytea);

CREATE OR REPLACE FUNCTION set_block_day_state(
	p_block_id uuid,
	p_month date,
	p_day_of_month integer,
	p_day_payload_hex text
) RETURNS bytea AS $$
DECLARE
	v_state bytea;
	v_day_payload bytea := decode(p_day_payload_hex, 'hex');
	v_day_index integer := p_day_of_month - 1;
	v_new_length integer := length(v_day_payload);
...
BEGIN
	...
	v_state := set_byte(decode('00', 'hex'), 0, 1) || v_new_offsets || v_before || v_day_payload || v_after;
